/* Shared behaviour: time helpers, availability engine, header, menu, offer popup, lightbox, hours. */
(function () {
  const RB = window.RB;
  ...
  /* ---------- Time (all shop-local, Harare is a fixed UTC+2) ---------- */
  const time = {
    now() {
  ...
  RB.time = time; RB.avail = avail; RB.store = store; RB.toast = toast; RB.copy = copy; RB.openLayer = openLayer; RB.closeLayer = closeLayer; RB.trap = trap;
